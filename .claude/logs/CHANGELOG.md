...
- Что сделано: по логам Nginx и loopback-проверкам установлено — запросы Gcore edge (92.223.76.20,
  ...
  (в логе 200, отдано ~250 КБ, соединение закрывает клиент) → файл не кэшируется → 504 наружу.
  Origin здоров: полный файл по HTTPS за 0.062с, Range ок, error.log пуст, ufw 443 открыт, fail2ban
  Gcore не банит, /media — статика с диска (не прокси), limit_req только на /api. Вывод (а): причина
  на стороне Gcore (edge→origin pull timeout/сегментация/throughput). Ничего на сервере не менялось.
- Рекомендация: залить видео в Gcore Storage (push, без origin-pull) / поднять origin timeout +
  shielding / облегчить ассет; origin host держать = gpeters.ru (под сертификат). Фикс — отдельной задачей.
- Файлы: `.claude/reports/REPORT_TASK-074_2026-06-09.md`, `.claude/tasks/SPRINT.md` (TASK-074 → [x]).
- Сборка/линт/типы: N/A (только диагностика).
- Дельта размера: 0.
...
- Что сделано: обновлён бинарник `public/images/expert-circle-hero.jpg` (новый кроп от Cowork,
  ...
  Прод: raw-картинка 200, размер 41511 и SHA1 совпадают с локальным файлом (новый кроп живой
  в шапке и на всех program-hero, т.к. файл один). Скрин приложен.
- Файлы: `antiage-platform/public/images/expert-circle-hero.jpg`,
  `.claude/reports/assets/REPORT_TASK-060b_hero.jpeg`.
- Сборка/линт/типы: без изменений кода (только бинарник).
- Дельта размера: −710 байт (новый кроп).
